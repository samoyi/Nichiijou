...
exports.listen = function(server) {
    ...
    io.sockets.on('connection', (socket)=>{  //定义每个用户连接的处理逻辑
        ...
        handleRoomJoining(socket);//处理用户聊天室的创建和变更
        socket.on('rooms', function() {  //当用户发出请求时，向其提供已经被占用的聊天室的列表
            // socket.emit('rooms', io.sockets.manager.rooms);
            socket.emit('rooms', io.of('/').adapter.rooms); // https://github.com/socketio/socket.io/issues/2428
        });
...
function joinRoom(socket, room) {
    ...
    let usersInRoom = io.of('/').in(room).clients; // https://github.com/socketio/socket.io/issues/2428
